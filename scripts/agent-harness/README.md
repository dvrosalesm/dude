...
| `url --subagent <id> [--workspace <id>]` | Open URL for browser MCP |
| `workspaces list --subagent <id>` | List SQLite workspaces |
| `workspaces create --subagent <id> --name "..."` | Create workspace + print `openUrl` |
| `chat --subagent <id> --message "..." [--workspace <id>]` | API-only agent turn (waits for completion) |
...
```sh
npm run agent -- workspaces create --subagent document-writer --name "Agent smoke"
# copy workspace id from output

npm run agent -- chat --subagent document-writer \
...
- `scripts/ui-harness/` — subagent-specific probes (Document Writer today)
- `packages/subagent-document-writer/src/ui/dev-debug.ts` — `window.__DUDE_DW_DEBUG__`
