 ...
 *
 * Used to hand off artifacts between specialists in a GT project session:
 * the main assistant remembers which workspace belongs to which specialist
 * (in its `gtSession` collection), and downstream specialists can pull
...
export function createReadSpecialistArtifactTool(): ToolDefinition {
  return {
    name: "read_specialist_artifact",
    label: "Read Specialist Artifact",
    description:
      "Read artifacts from another specialist's workspace by ID. " +
      "Use this when chaining specialists and a downstream task needs " +
      ...
      "from the `context` argument passed when this specialist was invoked. " +
    ...
    parameters: Type.Object({
      workspaceId: Type.String({
        description:
          ...
          "or the context provided to this specialist call — do not invent IDs.",
      }),
