...
export interface SubagentMessageBubbleProps {
  subagentId: string;
  /** Original message GT sent to the subagent — surfaced in the expanded view. */
  task: string;
...
/**
 * Renders a subagent hand-off as a secondary speaker in the chat —
 * avatar + flat content, no card frame or pre-title label.
 */
export function SubagentMessageBubble({
  subagentId,
  task,
  status,
  steps,
  result,
  onImageClick,
}: SubagentMessageBubbleProps) {
  const [expanded, setExpanded] = useState(false);
  const meta = SUBAGENT_META[subagentId];
  if (!meta) return null;

  const SpecIcon = meta.icon;
  const answer = extractSubagentAnswer(result);
  const imageUrl = extractImageUrl(result);
  const subagentHref = subagentListPath(meta.path);
          ...
          <div className={cn(chatAssistantBubbleClassName(), "space-y-2")}>
            ...
            {status === "failed" && (
              <p className="text-sm text-destructive/90">
                Something went wrong while running this subagent. Try again or
                open the workspace to investigate.
              </p>
            )}
          </div>

          {status === "completed" && (hasContent || subagentHref) && (
