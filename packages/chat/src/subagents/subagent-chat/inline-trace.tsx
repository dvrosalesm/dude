...
import { isSubagentCall, SUBAGENT_META } from "../subagent-meta";
...
export function InlineTrace({
  ...
  // Only show non-subagent tools here — subagent calls render separately
  const regularExecs = traces.flatMap((t) =>
    (t.toolExecutions ?? []).filter((e) => !isSubagentCall(e.tool)),
  );
  ...
  // If there's nothing to show (only subagent calls), skip
          ...
          {traces.map((trace) => (
            <div key={trace.id} className="space-y-2.5">
              ...
              {(trace.toolExecutions ?? [])
                .filter((e) => !isSubagentCall(e.tool))
                .map((exec, i) => {
