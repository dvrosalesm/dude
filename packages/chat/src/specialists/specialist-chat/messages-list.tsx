...
import { SpecialistMessageBubble } from "../specialist-message-bubble";
import {
  groupMessageTurns,
  turnPreviewText,
  type MessageTurn,
...
export function MessagesList({
  ...
  return (
    <>
        ...
        return (
          ...
          >
            ...
            {!isUser && message.executionTrace && (() => {
              const specCalls = (message.executionTrace!.toolExecutions ?? []).filter((e) => isSpecialistCall(e.tool));
              return (
                <div className="mt-3 space-y-3">
                  {specCalls.map((exec, i) => (
                    <SpecialistMessageBubble
                      key={`msg-spec-${index}-${i}`}
                      specialistId={exec.tool}
            ...
            {!isUser && !sending && (() => {
              ...
              if (!allSuggestions.length && message.executionTrace) {
                const wsToolExec = (message.executionTrace.toolExecutions ?? []).find(
                  (e) => e.tool === "list-specialist-workspaces" && e.result != null,
                );
                if (wsToolExec) {
                  try {
              ...
              return (
                <div className={cn(suggestionRowClass, isWorkspace && "mx-auto w-full max-w-2xl")}>
                  {allSuggestions.map((suggestion: string) => {
                    ...
                    if (parsed) {
                      // No handler wired on this surface (e.g. specialist
                      ...
                      // Pick an icon hint per action key. open-specialist
                      // opts into the specialist's own icon and color so
                      ...
                      if (parsed.action === "open-specialist") {
                        const specMeta = SPECIALIST_META[parsed.args[0] || ""];
                        if (specMeta) {
      ...
      {/* Live specialist delegations — rendered as the specialist's own
          chat bubbles so the user can see exactly which specialist is
      ...
      {(sending || wakingUp) && Array.isArray(executionTraces) && executionTraces.length > 0 && (() => {
        const liveSpecCalls = executionTraces.flatMap((trace) =>
          (trace.toolExecutions ?? []).filter((e) => isSpecialistCall(e.tool)),
        );
