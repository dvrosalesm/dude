...
import { specialistListPath, specialistWorkspacePath } from "@dude/workspaces/routes";
import { LavaLampPattern } from "../specialists/lava-lamp-pattern";
import { WorkspaceLayoutIcon, getSpecialistColor } from "../specialists/workspace-layout-icon";

interface SpecialistWorkspaceListProps {
...
export function SpecialistWorkspaceList({
  specialistId,
  title,
  subtitle,
  emptyTitle,
...
}: SpecialistWorkspaceListProps) {
  const router = useRouter();
  const workspaceRoute = specialistListPath(specialistId);
  ...
  useEffect(() => {
    ...
    async function loadWorkspaces() {
      ...
      try {
        const workspaces = await listSpecialistWorkspaces(specialistId);
        if (cancelled) return;
  ...
  async function handleCreateWorkspace() {
    ...
    try {
      ...
      const workspace = await createSpecialistWorkspace(specialistId, {
        name: createName.trim(),
        ...(Object.keys(configurations).length ? { configurations } : {}),
      });
      setIsCreateOpen(false);
      router.push(specialistWorkspacePath(specialistId, workspace.id));
    } catch (error) {
      const message =
              ...
              <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-6">
                {workspaces.map((workspace) => (
                  <Link
                    ...
                    href={specialistWorkspacePath(specialistId, workspace.id)}
                    className="block group"
                  >
                    <div className="rounded-2xl bg-card shadow-sm overflow-hidden">
                      <div className="relative h-24 overflow-hidden">
                        <LavaLampPattern id={String(workspace.id)} tint={getSpecialistColor(specialistId)} />
                        <div className="absolute inset-0 z-[1] flex items-center justify-center pointer-events-none">
                          <WorkspaceLayoutIcon
                            specialistId={specialistId}
                            className="w-32 h-20"
                            style={{ color: getSpecialistColor(specialistId) }}
                          />
                        </div>
