...
import type { AssistantTurnPayload } from "./specialist-chat-types";

export function uiMessageToSpecialistMessage(message: UiMessage): SpecialistMessage {
  return {
    id: message.id,
    role: message.role,
...
export function workspaceBootKey(specialistId: string, workspaceId: string): string {
  return `${specialistId}:${workspaceId}`;
}
