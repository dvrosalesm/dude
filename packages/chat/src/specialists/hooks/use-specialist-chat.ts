...
import type { SpecialistMessage, ChatImageAttachment, FileReference } from "../types";
import { attachAndUploadImages } from "./upload-chat-images";
import type { UseSpecialistChatConfig } from "./specialist-chat-types";
import { buildAttachedFilePromptBlock, buildChatHistory } from "./build-chat-history";
import {
  appendAssistantMessage,
  applyAssistantTurnResult,
  bootedWorkspaceKeys,
  uiMessageToSpecialistMessage,
  workspaceBootKey,
} from "./specialist-chat-message-utils";

export type {
  UseSpecialistChatConfig,
  SpecialistChatSendTurnInput,
  SpecialistChatSendTurnResult,
} from "./specialist-chat-types";

export function useSpecialistChat(config: UseSpecialistChatConfig) {
  const {
    specialistId,
    workspaceId,
    messages,
  ...
  const session = useGatewaySession(specialistId, workspaceId);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (reloaded.length > 0) {
        setMessages(reloaded.map(uiMessageToSpecialistMessage));
      }
      ...
      try {
        ...
        if (!cancelled) {
          setMessages(fresh.map(uiMessageToSpecialistMessage));
          onAssistantResponse?.();
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const bootKey = workspaceBootKey(specialistId, workspaceId);
  ...
  }, [workspaceId, specialistId, messagesLoaded, messages.length, bootMessage]);
  ...
  async function runVisibleTurn(params: {
    ...
    optimisticUser: SpecialistMessage;
  }) {
    const controller = new AbortController();
    abortRef.current = controller;
  ...
  async function handleSendMessage(messageOverride?: string) {
    ...
    abortRef.current?.abort();
    // #region agent log
    fetch('http://127.0.0.1:7884/ingest/44760fdd-2433-4958-be9a-fbf49e3e279f',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'099559'},body:JSON.stringify({sessionId:'099559',location:'use-specialist-chat.ts:send:start',message:'send started',data:{specialistId,workspaceId,textLen:text.trim().length},timestamp:Date.now(),hypothesisId:'F'})}).catch(()=>{});
    // #endregion
  ...
  function handleStopSending() {
    ...
    clearActiveJobId();
    if (workspaceId) {
      abandonAssistantTurn(specialistId as SpecialistId, workspaceId);
    }
    setSending(false);
    setWakingUp(false);
    // #region agent log
    fetch('http://127.0.0.1:7884/ingest/44760fdd-2433-4958-be9a-fbf49e3e279f',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'099559'},body:JSON.stringify({sessionId:'099559',location:'use-specialist-chat.ts:stop',message:'user stopped turn',data:{specialistId,workspaceId},timestamp:Date.now(),hypothesisId:'F'})}).catch(()=>{});
    // #endregion
  }
  ...
  async function handleClearChat() {
    ...
    if (clearThread) {
      ...
      bootedRef.current = false;
      if (workspaceId) {
        bootedWorkspaceKeys.delete(workspaceBootKey(specialistId, workspaceId));
      }
      return;
    }
    setMessages([]);
    ...
    if (workspaceId) {
      void clearAssistant(specialistId, workspaceId);
    }
  }
