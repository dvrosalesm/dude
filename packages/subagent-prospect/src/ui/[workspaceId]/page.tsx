...
function isRawSubagentMessage(value: unknown): value is {
  role: "user" | "assistant";
  message: string;
  date?: string;
  created_at?: string;
...
export default function ProspectWorkspacePage() {
  ...
  const normalizeMessages = useCallback((raw: unknown[]): SubagentMessage[] => {
    return (Array.isArray(raw) ? raw : [])
      .filter(isRawSubagentMessage)
      .map((m) => {
  ...
  return (
    <SubagentWorkspaceFrame>
      <WorkspaceHeaderBar
        title={workspaceName}
        backHref={subagentListPath("prospect")}
      ...
      <SubagentWorkspaceMain className="flex-row">
