 ...
 *
 * Internally still routed via the gateway's specialist-run path with the
...
export function createGenerateImageTool(): ToolDefinition {
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      const result = await internalPost("/assistant/specialist-run", {
        specialistId: "image-studio",
        orgId: scopeId,
