...
export function useReports({
  ...
  async function loadSettingsFromDb() {
    ...
    try {
      const data = (await callWorkspaceAction(subagentId, workspaceId, "settings", {
        method: "GET",
  ...
  async function saveSettingsToDb(nextColors: string[]) {
    ...
    try {
      ...
      await callWorkspaceAction(subagentId, workspaceId, "settings", {
        method: "POST",
        body: {
  ...
  async function loadReportsFromDb() {
    if (!workspaceId) return;
    try {
      const data = (await callWorkspaceAction(subagentId, workspaceId, "reports", {
        method: "GET",
  ...
  async function saveReportsToDb(nextReports: ReportItem[]) {
    if (!workspaceId) return;
    try {
      await callWorkspaceAction(subagentId, workspaceId, "reports", {
        method: "POST",
        body: {
          reports: nextReports,
        },
