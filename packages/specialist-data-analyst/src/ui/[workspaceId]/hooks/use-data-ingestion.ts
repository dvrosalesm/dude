...
export function useDataIngestion({
  ...
  async function submitDirectDatabase(action: "test" | "save") {
    ...
    try {
      ...
      await callWorkspaceAction(specialistId, workspaceId, "connections", {
        method: "POST",
        body: {
          action,
          connection: {
  ...
  async function handleImport() {
    ...
    try {
      if (!workspaceId) throw new Error("Missing workspace id");
      const { uploadKey } = await postWorkspaceUploadUrl(specialistId, workspaceId, ingestFile);
      ...
      for await (const event of streamWorkspaceIngest(specialistId, workspaceId, {
        uploadKey,
